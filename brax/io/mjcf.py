...
def load_model(mj: mujoco.MjModel) -> System:
    ...
    if any(i not in [0, 1] for i in mj.actuator_biastype):
        raise NotImplementedError(
            'Only actuator_biastype in [0, 1] are supported.')
    ...
    for _, group in itertools.groupby(
        ...
        if not (position == position[0]).all():
            raise RuntimeError(
                'invalid joint stack: only one joint position allowed')
    ...
    for typ, axis, limit, stiffness in zip(
        ...
        if typ == 0:
            ...
            if stiffness > 0:
                raise RuntimeError(
                    'brax does not support stiffness for free joints')
            stiffness = np.zeros(6)
        elif typ == 1:
            motion = Motion(ang=np.eye(3), vel=np.zeros((3, 3)))
            if np.any(~np.isinf(limit)):
                raise RuntimeError(
                    'brax does not support joint ranges for ball joints')
    ...
    def key_fn(g, m): return (jax.tree_map(np.shape, g), m)
    ...
    for i, typ in enumerate(mj.geom_type):
        ...
        if typ == 0:  # Plane
            ...
            geom_groups.setdefault(key_fn(geom, 0), []).append(
                geom)  # visual only
            if custom['convex'][i]:
    ...
    ctrl_range[~(mj.actuator_ctrllimited == 1),
    ...
    if (params_contact[0] != params_contact).any():
        raise NotImplementedError(
            'brax only supports one contact solver params')
    ...
    actuator_types = ''.join([_ACT_TYPE_STR[bt]
                             for bt in mj.actuator_biastype])
    ...
    if 'f' in link_types:
        free_idx = np.array(
            [i for i, typ in enumerate(link_types) if typ == 'f'])
